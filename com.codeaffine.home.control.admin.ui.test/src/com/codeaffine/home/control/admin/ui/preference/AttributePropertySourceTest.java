...
public class AttributePropertySourceTest {
  ...
  @Before
  public void setUp() {
    info = stubPreferenceInfo( ATTRIBUTE_NAME, stubAttributeDescriptor( Integer.class ) );
    propertySource = new AttributePropertySource( info );
  }
  ...
  private static PreferenceInfo stubPreferenceInfo(
    String attributeName, PreferenceAttributeDescriptor attributeDescriptor )
  {
    PreferenceInfo result = mock( PreferenceInfo.class );
    when( result.getAttributeDescriptor( attributeName ) ).thenReturn( attributeDescriptor );
    when( result.getAttributeDescriptors() ).thenReturn( asList( attributeDescriptor ) );
    return result;
  }
}